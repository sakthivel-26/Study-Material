...
export default function MockTestsPage() {
  ...
  const visibleTests = selected ? mockTests.filter(t => selected.groups.includes(testGroup(t))) : mockTests;
  ...
  const start = (test) => { const free = firstForCategory(test) && !claimed[test.category]; if (!free && !paid[packageId]) { setNotice(`Your free ${test.category} mock has already been used. Unlock the complete ${selected?.label || testGroup(test)} package for ₹${amount}.`); return; } if (free) { const next={...claimed,[test.category]:test.id};localStorage.setItem(claimedKey,JSON.stringify(next));setClaimed(next); } setActiveTest(test); };
  return <><PageHeader icon={<ClipboardList size={22}/>} title={selected ? `${selected.label} Mock Tests` : "Mock Tests"} subtitle={selected ? `Only ${selected.label} tests are displayed. First mock is free; full package ₹${amount}.` : "Choose an exam course to view its dedicated mock-test series."}/>
    {selected && <button onClick={()=>setSearchParams({})} className="btn-ghost text-sm px-3 py-2 mb-5"><ArrowLeft size={15}/>All exam packages</button>}
