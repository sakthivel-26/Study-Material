...
export function AppProvider({ children }) {
  ...
  const [notifications, setNotifications] = useState(useRealtimeBackend ? [] : NOTIFICATIONS);
  const [mockTests, setMockTests] = useState(useRealtimeBackend ? [] : MOCK_TESTS);
  ...
  // Save uploads to localStorage in demo mode
  useEffect(() => {
    if (!useRealtimeBackend) {
      try {
        localStorage.setItem("ken_ias_uploads", JSON.stringify(uploads));
      } catch (err) {
        console.warn("Could not save uploads to localStorage", err);
      }
    }
  }, [uploads]);
  ...
  const pushNotification = (type, title, body) => {
    ...
    const n = { id: Date.now(), type, icon: icons[type] || "🔔", title, body, time: "just now", read: false, color: colors[type] || "#6D28D9" };
