    ...
    <div className="relative hidden lg:flex lg:w-[45%] bg-hero text-white overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col p-10 w-full">
        ...
        <div className="flex-1 flex flex-col justify-center max-w-sm">
          ...
          >
            Crack TNPSC, Banking, SSC &amp; Railway exams.
          </motion.h2>
          <motion.p
            initial={{ opacity: 0, y: 16 }}
            animate={{ opacity: 1, y: 0 }}
...
export function SignupPage() {
  ...
  const [form, setForm] = useState({ name: "", email: "", phone: "", password: "", confirm: "", enrolled: "IBPS PO / Clerk" });
  ...
  const submit = async (e) => {
    ...
    if (!form.phone.trim() || form.phone.replace(/\D/g, '').length < 10) return setLocalErr("Please enter a valid 10-digit mobile number.");
    ...
    try {
      const r = await signUp({ name: form.name, email: form.email, phone: form.phone, password: form.password, role: "student", enrolled: form.enrolled });
        ...
        <div className="w-full max-w-md">
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="card p-7 sm:p-9 shadow-soft">
            ...
            <form onSubmit={submit} className="space-y-4 mt-4">
              <Field icon={User} label="Full name" value={form.name} onChange={(v) => set("name", v)} placeholder="Full Name" autoComplete="name" />
              ...
              <Field icon={Phone} label="Mobile number" type="tel" value={form.phone} onChange={(v) => set("phone", v)} placeholder="98765 43210" autoComplete="tel" />
