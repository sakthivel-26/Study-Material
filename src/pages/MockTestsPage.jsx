...
export default function MockTestsPage() {
  ...
  const hasAdminAccess = Array.isArray(user?.access) ? user.access.includes(packageId) : user?.access === "academy";
  const start = (test) => { 
    if (!isAuthed) {
    ...
    if (!free && !paid[packageId] && !hasAdminAccess) { 
      setNotice(`Your free ${test.category} mock has already been used. Unlock the complete ${selected?.label || testGroup(test)} package for ₹${amount}.`); 
      return; 
    } 
    if (free) { 
  ...
  return <><PageHeader icon={<ClipboardList size={22}/>} title={exam ? `${exam} Mock Tests` : selected ? `${selected.label} Mock Tests` : "Mock Tests"} subtitle={selected ? `Only ${selected.label} tests are displayed. First mock is free; full package ₹${amount}.` : "Choose an exam course to view its dedicated mock-test series."}/>
    ...
    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-10">{visibleTests.length===0?<div className="card p-7 text-sm text-ink-muted">No {selected?.label || ""} mock tests have been published yet. The admin can create one from the matching category.</div>:visibleTests.map((t,i)=>{const free=firstForCategory(t)&&!claimed[t.category];const unlocked=!!paid[packageId] || hasAdminAccess;return <motion.div key={t.id} initial={{opacity:0,y:14}} animate={{opacity:1,y:0}} transition={{delay:i*.06}} className="card card-hover p-5 flex flex-col" style={{borderTop:`4px solid ${t.color}`}}><div className="flex justify-between gap-2"><span className="chip self-start mb-3" style={{background:`${t.color}1A`,color:t.color}}>{t.category}</span><span className={`chip ${free?"bg-emerald-50 text-emerald-700":"bg-slate-100 text-slate-600"}`}>{hasAdminAccess?"Academy access":unlocked?"Package unlocked":free?"First mock free":<><LockKeyhole size={12}/>₹{amount} package</>}</span></div><h3 className="font-bold text-ink mb-2">{t.title}</h3><div className="flex items-center gap-4 text-xs text-ink-muted mb-4"><span className="flex items-center gap-1"><HelpCircle size={13}/>{t.questions} Qs</span><span className="flex items-center gap-1"><Clock size={13}/>{t.time}</span><span className="flex items-center gap-1"><TrendingUp size={13}/>{t.taken?.toLocaleString()} taken</span></div><button onClick={()=>start(t)} className={`mt-auto text-sm py-2.5 ${(free||unlocked)?"btn-primary":"btn-ghost"}`}>{(free||unlocked)?<Play size={15}/>:<LockKeyhole size={15}/>}{unlocked?"Start test":free?"Start free test":`Unlock for ₹${amount}`}</button></motion.div>})}</div>{activeTest&&<MockTestRunner test={activeTest} onClose={()=>setActiveTest(null)}/>}{checkout && selected && <PaymentCheckout packageId={packageId} packageName={selected.label} amount={amount} onClose={()=>setCheckout(false)} onSuccess={async ()=>{const next={...paid,[packageId]:true};localStorage.setItem("ken_paid_packages",JSON.stringify(next));setPaid(next);setCheckout(false);setNotice("Payment verified. Your package is now unlocked."); if (isAuthed && user?.id && useRealtimeBackend) { try { await fsUpdateUserPurchases(user.id, packageId); } catch(err) { console.error("Firebase sync error", err); } }}}/>}</>;
}
